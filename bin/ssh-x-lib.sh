...
ssh::add_key() {
  ...
  SSH_ADD_COMMAND="SSH_ASKPASS_REQUIRE=require SSH_ASKPASS=$SSH_ASKPASS SSH_X_PASSPHRASE=$SSH_X_PASSPHRASE ssh-add -t $SSH_X_LIFE $FD_PRIVATE_KEY 2>&1"
...
ssh::agent_state_human() {
  ...
  case $SSH_X_AGENT_RUN_STATE in
    0)
      # SSH_AGENT_PID env may show a wrong value
      SSH_AGENT_PID=$(pgrep ssh-agent)
      echo "Agent running with key on the PID $SSH_AGENT_PID"
      return
      ;;
    1)
      # SSH_AGENT_PID env may show a wrong value
      SSH_AGENT_PID=$(pgrep ssh-agent)
      echo "Agent running without key on the PID $SSH_AGENT_PID"
      return
      ;;
    2)
...
ssh::agent_kill() {
  ...
  if ! OUTPUT=$(ssh-agent -k 2>&1); then
    if [[ $OUTPUT == *"No such process"* ]]; then
      unset SSH_AGENT_PID
      ssh::agent_kill
      return
    fi
    echo::err "Error while trying to kill the agent: $OUTPUT"
  else
    echo::info "Agent $SSH_AGENT_PID killed"
  fi
...
ssh::get_identity() {
  ...
  KEY=$(ssh::get_conf "identity" "$DESTINATION")
...
#
# @args $1 - the conf
# @args $2 - the destination ie ($USER@$HOST)
# @stdout - the value
ssh::get_conf() {
  ...
  SSH_ARGS=(-T -G "$DESTINATION")
  $SSH_PATH "${SSH_ARGS[@]}" | grep "$CONF" | awk '{ print $2 }'
